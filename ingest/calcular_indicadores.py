...
# ---------- 7. Crear tablas (si no existen) ---------------------
create_nac = """
CREATE TABLE IF NOT EXISTS enemdu.indicadores_nacionales (
    "Año" UInt16, "Periodo" UInt8, "Mes" String,
    "TPG (%)" Float32, "TPB (%)" Float32, "TD (%)" Float32,
    "Empleo Total (%)" Float32, "Formal (%)" Float32, "Informal (%)" Float32,
    "Adecuado (%)" Float32, "Subempleo (%)" Float32,
    "No Remun. (%)" Float32, "Otro No Pleno (%)" Float32,
    "Brecha Adecuado H-M (%)" Float32, "Brecha Salarial H-M (%)" Float32,
    "NiNi (%)" Float32, "Desempleo Juvenil (%)" Float32,
    "Trabajo Infantil (%)" Float32, "Manufactura / Empleo (%)" Float32
) ENGINE=MergeTree ORDER BY (\"Año\",\"Periodo\")
"""

create_city = """
CREATE TABLE IF NOT EXISTS enemdu.indicadores_por_ciudad (
    "Año" UInt16, "Periodo" UInt8, "Mes" String, "Ciudad" FixedString(6),
    "TPG (%)" Float32, "TPB (%)" Float32, "TD (%)" Float32,
    "Empleo Total (%)" Float32, "Formal (%)" Float32, "Informal (%)" Float32,
    "Adecuado (%)" Float32, "Subempleo (%)" Float32,
    "No Remun. (%)" Float32, "Otro No Pleno (%)" Float32,
    ...
    "NiNi (%)" Float32, "Desempleo Juvenil (%)" Float32,
    "Trabajo Infantil (%)" Float32, "Manufactura / Empleo (%)" Float32
) ENGINE=MergeTree ORDER BY (\"Año\",\"Periodo\",\"Ciudad\")
"""

client.execute(create_nac)
client.execute(create_city)

# ---------- 8. Cargar CSV en ClickHouse -------------------------
def load_csv(path: Path, table: str):
    with path.open("rb") as fh:
        client.execute(
            f"INSERT INTO {table} FORMAT CSVWithNames",
            fh.read()
        )

load_csv(file_nac,  "enemdu.indicadores_nacionales")
load_csv(file_city, "enemdu.indicadores_por_ciudad")

print("✅ Indicadores cargados en ClickHouse")
