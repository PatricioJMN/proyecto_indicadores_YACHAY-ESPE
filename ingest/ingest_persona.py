...
def main():
    ...
    for csv_path in Path(DATA_DIR).glob('*.csv'):
        ...
        batch_values = [tuple(coerce_value(name, row.get(name, None)) for name, _, _ in columns_meta) for _, row in df.iterrows()]
        ...
        try:
            ...
            log(f"✓ {csv_path.name} — {total} filas insertadas.")
        except Exception as e:
            log(f"[ERROR-BATCH] Falló batch: {e}")
            ...
            log(f"→ {csv_path.name}: {ok_cnt} OK, {fail_cnt} fallidas.")
